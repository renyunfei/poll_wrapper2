...
/* Add fd with interest events. Fails if full or fd already exists (fd must be unique). */
...
/* Remove an existing fd from the loop (compacts by swapping last entry). */
...
/* Get revents by fd after pw_wait; returns 0 if fd not found/invalid loop. */
